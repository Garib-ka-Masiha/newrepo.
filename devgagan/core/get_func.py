...
from pyrogram.errors import FloodWait, ChannelBanned, ChannelInvalid, ChannelPrivate, ChatIdInvalid, ChatInvalid
from devgagan import app

async def progress_bar(current, total, status, edit, start):
    now = time.time()
    diff = now - start
...
"""
    try:
        await edit.edit_text(progress_text)
    except:
        pass
...
def time_formatter(seconds):
    ...
    return (f"{days}d " if days else "") + (f"{hours}h " if hours else "") + (f"{minutes}m " if minutes else "") + (f"{seconds}s" if seconds else "")

async def get_msg(app, sender, edit_id, msg_link, i, message, thumb=None):
    try:
        ...
        if not file:
            await app.edit_message_text(sender, edit_id, "❌ Download failed.")
            return

        caption = msg.caption or "Here is your file! ✅"

        if file_size > size_limit:
            await app.edit_message_text(sender, edit_id, "**❌ File too big.**")
            return

        if file and os.path.exists(file):
            if msg.video or file.endswith((".mp4", ".mkv", ".webm")):
                await app.send_video(
                    chat_id=sender,
                    video=file,
                    caption=caption,
                    thumb=thumb,
                    supports_streaming=True  # ✅ enable proper video streaming
                )
            elif msg.audio:
        ...
        await app.edit_message_text(sender, edit_id, "❗ Channel inaccessible error")
    except FloodWait as e:
        await asyncio.sleep(e.x)
        await get_msg(app, sender, edit_id, msg_link, i, message, thumb)
    except Exception as e:
