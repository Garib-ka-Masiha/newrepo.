...
#     return 0 if user_id in await premium_users() or user_id in ([OWNER_ID] if isinstance(OWNER_ID, int) else OWNER_ID) else 1
   ...
   if update_channel:
      try:
         ...
         if user.status == "kicked":
            await message.reply_text("You are Banned. Contact -- @NEXUZ_ELITE_BOT")
            return 1
      except UserNotParticipant:
        caption = f"Join our channel to use the bot"
        await message.reply_photo(photo="https://envs.sh/dTd.jpg",caption=caption, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Join Now...", url=f"{url}")]]))
        return 1
      except Exception:
         await message.reply_text("Something Went Wrong. Contact us @NEXUZ_ELITE_BOT...")
         return 1
