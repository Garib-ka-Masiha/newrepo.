...
async def myplan(client, message):
    ...
    if data and data.get("expire_date"):
        ...
        await message.reply_text(f"⚜️ ᴘʀᴇᴍɪᴜᴍ ᴜꜱᴇʀ ᴅᴀᴛᴀ :\n\n👤 ᴜꜱᴇʀ : {user}\n⚡ ᴜꜱᴇʀ ɪᴅ : <code>{user_id}</code>\n⏰ ᴛɪᴍᴇ ʟᴇꜰᴛ : {time_left_str}\n⌛️ ᴇxᴘɪʀʏ ᴅᴀᴛᴇ : {expiry_str_in_ist}\n\n⚡️ᴘʟᴀɴ ᴛʏᴘᴇ : {user_type}")
    else:
        await message.reply_text(
        f"ʜᴇʏ {user},\n\nʏᴏᴜ ᴅᴏ ɴᴏᴛ ʜᴀᴠᴇ ᴀɴʏ ᴀᴄᴛɪᴠᴇ ᴘʟᴀɴs",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("💎 View Subscription Plans", callback_data="subscribe_plans")],
            [InlineKeyboardButton("⬅️ Back", callback_data="delete_msg")]
        ])
    )
...
async def callback_handler(client, cb: CallbackQuery):
    ...
    if data == "subscribe_plans":
        await cb.answer("Showing plans...")  # Replace with actual logic
...
async def give_premium_cmd_handler(client, message):
    if len(message.command) == 5: # to take type
        time_zone = datetime.datetime.now(pytz.timezone("Asia/Kolkata"))
        current_time = time_zone.strftime("%d-%m-%Y\n⏱️ ᴊᴏɪɴɪɴɢ ᴛɪᴍᴇ : %I:%M:%S %p") 
        user_id = int(message.command[1])
        user = await client.get_users(user_id)
        time = message.command[2]+" "+message.command[3]
        user_type = message.command[4].lower()
        allowed_types = ["diamond", "gold", "silver" , "premium"]
        if user_type.lower() not in allowed_types:
            await message.reply_text("Invalid user type. Allowed types: premium, vip, gold, silver.")
            return
        seconds = await get_seconds(time)
        if seconds > 0:
            expiry_time = datetime.datetime.now() + datetime.timedelta(seconds=seconds)  
            await plans_db.add_premium(user_id, expiry_time,user_type)
            data = await plans_db.check_premium(user_id)
            expiry = data.get("expire_date")   
            expiry_str_in_ist = expiry.astimezone(pytz.timezone("Asia/Kolkata")).strftime("%d-%m-%Y\n⏱️ ᴇxᴘɪʀʏ ᴛɪᴍᴇ : %I:%M:%S %p")         
            await message.reply_text(f"ᴘʀᴇᴍɪᴜᴍ ᴀᴅᴅᴇᴅ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ ✅\n\n👤 ᴜꜱᴇʀ : {user.mention}\n⚡ ᴜꜱᴇʀ ɪᴅ : <code>{user_id}</code>\n⏰ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇꜱꜱ : <code>{time}</code>\n\n⏳ ᴊᴏɪɴɪɴɢ ᴅᴀᴛᴇ : {current_time}\n\n⌛️ ᴇxᴘɪʀʏ ᴅᴀᴛᴇ : {expiry_str_in_ist}\n\n⚡️ᴘʟᴀɴ ᴛʏᴘᴇ : {user_type}\n\n__**Powered by Team NEXUZ__**", disable_web_page_preview=True)
            await client.send_message(
                chat_id=user_id,
                text=f"👋 ʜᴇʏ {user.mention},\nᴛʜᴀɴᴋ ʏᴏᴜ ꜰᴏʀ ᴘᴜʀᴄʜᴀꜱɪɴɢ ᴘʀᴇᴍɪᴜᴍ.\nᴇɴᴊᴏʏ !! ✨🎉\n\n⏰ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇꜱꜱ : <code>{time}</code>\n⏳ ᴊᴏɪɴɪɴɢ ᴅᴀᴛᴇ : {current_time}\n\n⌛️ ᴇxᴘɪʀʏ ᴅᴀᴛᴇ : {expiry_str_in_ist}\n\n⚡️ᴘʟᴀɴ ᴛʏᴘᴇ : {user_type}", disable_web_page_preview=True
            )
...
async def transfer_premium(client, message):
    if len(message.command) == 2:
        ...
        if data and data.get("_id"):  # Verify sender is already a premium user
            ...
            await plans_db.add_premium(new_user_id, expiry,user_type)
            ...
            await message.reply_text(
                ...
                f"⚡️ᴘʟᴀɴ ᴛʏᴘᴇ : {user_type}"
                f"__Powered by Team SUNG_ 🚀"
            )
